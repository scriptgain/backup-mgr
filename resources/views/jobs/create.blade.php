            ...
            <div x-show="tab==='schedule'" x-cloak x-data="{ cron: '{{ old('schedule_cron') }}' }">
                ...
                <div class="mt-5 border-t border-slate-100 pt-5">
                    <x-field label="Prune Schedule" for="prune_schedule_cron" :error="$errors->first('prune_schedule_cron')"
                        ...
                        <x-input id="prune_schedule_cron" name="prune_schedule_cron" :value="old('prune_schedule_cron')" placeholder="30 4 * * *" />
                    </x-field>
                </div>
                <div class="mt-5 space-y-4 border-t border-slate-100 pt-5">
                    <x-toggle name="prune_after_backup" :checked="true" label="Prune After Each Backup" description="Apply retention and run kopia maintenance after each run. Only expires snapshots that share a source; the prune schedule above is what retires the rest." />
